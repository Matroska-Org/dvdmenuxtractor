...
typedef struct {
  ...
  union {
    struct {
      ...
      unsigned BITFIELD_TYPE mc_intro           : 1; /* probably 0: true, 1:false */
      unsigned BITFIELD_TYPE mode               : 1; /* Karaoke mode 0: solo 1: duet */
#else
    ...
    struct {
      ...
      unsigned BITFIELD_TYPE dolby_encoded      : 1; /* suitable for surround decoding */
...
typedef struct {
  ...
  unsigned BITFIELD_TYPE playback_mode    : 1;  /**< When set, enter StillMode after each VOBU */
  unsigned BITFIELD_TYPE restricted       : 1;  /**< ?? drop out of fastforward? */
...
typedef struct {
  ...
  unsigned BITFIELD_TYPE multi_or_random_pgc_title : 1; /* 0: one sequential pgc title */
